...
class ClfTripletMarginLossOHNM(_Loss):
    r""" Triplet Margin Loss with Online Hard Negative Mining
    ...
    * Assumes diagonal entries are ground truth (for multi-class as of now)
    ...
    def __init__(self, reduction='mean', margin=0.8, k=3, apply_softmax=False):
        super(ClfTripletMarginLossOHNM, self).__init__(reduction=reduction)
        self.margin = margin
        self.k = k
        self.apply_softmax = apply_softmax
...
class HybridMultiLabelLoss(_Loss):
    r""" Triplet Margin Loss with Online Hard Negative Mining
    * Applies loss using the hardest negative in the mini-batch
    * Assumes diagonal entries are ground truth
    ...
    def __init__(self, margin=1.0, eps=1.0e-6, reduction='mean',
        ...
        super(HybridMultiLabelLoss, self).__init__(reduction)
        self.mx_lim = 100
        self.mn_lim = -100
        self.alpha = alpha
        self._eps = eps
